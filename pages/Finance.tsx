...
export const FinancePage: React.FC = () => {
    ...
    const rawFinancialItems = useMemo(() => {
        ...
        // --- Setup (Montagem) Fees ---
        const SETUP_RATE = 350;

        const processExtraFee = (
            instructorId: string | undefined,
            days: number | undefined,
            type: 'Montagem' | 'Desmontagem',
            suffix: string
        ) => {
            if (!instructorId || !days || days <= 0) return;

            // Security check
            if (isInstructor && instructorId !== currentUser.id) return;

            const instructorUser = users.find(u => u.id === instructorId);
            const uniqueId = `${cls.id}-${type}-${suffix}`;
            const paymentRecord = payments.find(p => p.scheduleItemId === uniqueId && p.instructorId === instructorId);
            const isPaid = !!paymentRecord;

            items.push({
                id: uniqueId,
                scheduleId: uniqueId, // Using uniqueId as scheduleId for payments
                instructorId: instructorId,
                instructorName: instructorUser?.name || 'Desconhecido',
                instructorRole: instructorUser?.role || 'Instrutor',
                classId: cls.id,
                className: cls.name,
                date: cls.startDate, // Use class start date for reference
                subject: type,
                modality: 'Prática', // Treated as practical work
                hours: days, // Storing DAYS in hours field (will handle in UI)
                rate: SETUP_RATE, // Daily rate
                value: days * SETUP_RATE,
                status: isPaid ? 'Pago' : 'Pendente',
                paymentDate: paymentRecord?.datePaid
            });
        };

        processExtraFee(cls.setupInstructor1Id, cls.setupInstructor1Days, 'Montagem', '1');
        processExtraFee(cls.setupInstructor2Id, cls.setupInstructor2Days, 'Montagem', '2');
        processExtraFee(cls.teardownInstructor1Id, cls.teardownInstructor1Days, 'Desmontagem', '1');
        processExtraFee(cls.teardownInstructor2Id, cls.teardownInstructor2Days, 'Desmontagem', '2');
                            ...
                            filteredItems.map((log) => (
                                <tr key={log.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-700">
                                        {log.hours} {log.subject === 'Montagem' || log.subject === 'Desmontagem' ? 'dias' : 'h'}
                                    </td>
